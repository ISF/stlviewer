...
# Shell completions. Generated by the freshly-installed CLI binary so they
# always match the actual flag set. fish is the only one wired up today;
# other shells follow the same pattern (just point at their conventional
...
install_completion_fish() {
    ...
    # Prefer the vendor dir so it's available to every user on the system.
    for candidate in \
    ...
    if [[ -z "$target_dir" ]]; then
        # Fall back to per-user — always writable; loaded by fish on startup.
        target_dir="${HOME}/.config/fish/completions"
        mkdir -p "$target_dir"
    fi
    target_file="${target_dir}/${APP_NAME}.fish"
    step "Installing fish completion → ${target_file}"
    "$CLI_DEST" --completions fish > "$target_file"
}
...
cat <<MSG
  ...
  In a new fish shell, \`stlviewer <TAB>\` should complete files
  and \`stlviewer --<TAB>\` should suggest flags.
